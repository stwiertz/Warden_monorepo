...
8. [x] **AC8 — Sprint-status flip on completion.** ✅ Post-merge follow-up (2026-06-11, `story-1-4-postmerge`): flipped `_bmad-output/sprint-status.yaml` `1-4-…: → done`; `last_updated` bumped 2026-06-11; `epic-1` stays `in-progress` (no epic flip). Prior in-file flip (`ready-for-dev → in-progress → review`, 2026-06-09) lived on `story-9-12-picker-fixes`'s entangled working tree alongside Story 1.3; this follow-up records the canonical `done` on `main` directly (1-4 line only — 1.3's flip stays with 1.3).

9. [x] **AC9 — Single-PR delivery; tiny post-merge follow-up per Two-PR pattern.** ✅ Delivered as local `git merge --no-ff` per [[feedback_two_pr_docs_execution]] + the 1.1/1.2/9.x precedent (`gh` unauthenticatable non-interactively on this host). **Main delivery:** the file-isolated 1.4 deliverable (`apps/mobile/{package.json,app.json,google-services.json,.env.example}` + `pnpm-lock.yaml` + this story file) was committed as `feat: firebase v12 RN auth migration — add deps + prebuild (Story 1.4)` (`50309fd`) and merged to `main` (`692629a`). **Post-merge follow-up (2026-06-11, `story-1-4-postmerge`):** this `review → done` flip + AC8/AC9 box-flips + the `### Review Findings` section + `deferred-work.md` entries, merged `--no-ff` to `main`. The 1.4 deliverable was kept clean of Story 1.3's still-uncommitted `apps/web/**` work by stashing the entangled `story-9-12-picker-fixes` tree before branching off `main` ([[project_warden_shared_doc_commit_boundary]] honored — 1.3's `sprint-status.yaml` flip stays with 1.3).
...
- [x] **Task 6: Commit, deliver, open PR (AC: 8, 9)** — _delivered: feature merge `692629a` on `main`; review-close via `story-1-4-postmerge` (2026-06-11)._
  - [x] Branched `story-1-4-firebase-rn-add-deps-prebuild` off `main`; feature commit `50309fd` merged to `main` (`692629a`).
  - [x] Single commit `feat: firebase v12 RN auth migration — add deps + prebuild (Story 1.4)`. Files (1.4-isolated): `apps/mobile/{package.json,app.json,google-services.json,.env.example}`, `pnpm-lock.yaml`, this story file. (`sprint-status.yaml` flip held out of the feature commit to respect 1.3's shared-doc boundary.)
  - [x] Flipped `_bmad-output/sprint-status.yaml` `1-4-...: → done` (post-merge follow-up; `last_updated` bumped 2026-06-11).
  - [x] Local `git merge --no-ff` (`gh` unauthenticatable) for both the feature and the `story-1-4-postmerge` follow-up (this `review → done` flip + Review Findings + deferrals).

### Review Findings

_Code review 2026-06-11 (claude-opus-4-8[1m], /bmad-code-review on merged commit `50309fd`). 3 adversarial layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor), 0 failed. Triage: 0 decision-needed, 0 patch, 4 defer, 9 dismissed — **no correctness defect in the merged plumbing**; deps, plugin wiring, asset placement, and AC6 (4 service files byte-identical) all verified. The 2 decision-needed items were resolved with Stephane: project-identity match is by-design (INVARIANT 2; unverifiable from committed env placeholders is inherent → dismissed); iOS asymmetry accepted as Phase 2 (→ defer). The 1 trivial patch (trailing newline on the byte-exact console download) dismissed to keep the asset faithful._

- [x] [Review][Defer] iOS Firebase config absent — `@react-native-firebase/{app,auth}` plugins registered platform-agnostically but no `GoogleService-Info.plist` / `ios.googleServicesFile` [apps/mobile/app.json] — deferred: iOS is Phase 2 (`architecture.md:630`); plist + `ios.googleServicesFile` + an accidental-iOS-prebuild guard land then.
- [x] [Review][Defer] App Check absent — committed `api_key` + OAuth IDs are safe for Firestore (rules gate reads) but leave the Auth/Identity-Toolkit surface unattested [apps/mobile/google-services.json] — deferred: security-hardening, out of 1.4 plumbing scope.
- [x] [Review][Defer] Two physical `firebase` copies under hoisting (app-direct 12.12.1 vs RNFB-transitive 12.14.0) — Metro binds one non-deterministically [pnpm-lock.yaml] — deferred: flag for **Story 1.5** (firebaseConfig migration); consider a root `pnpm.overrides` if a single firebase must be pinned.
- [x] [Review][Defer] `firestore.rules` confined to `apps/web/` with no deploy binding to `warden-8ce50` — mobile reads depend on it but the two evolve separately [apps/web/firestore.rules] — deferred: cross-app, pre-existing.
- [x] [Review][Defer] `.env.example` bucket placeholder uses old `…appspot.com` vs the new `…firebasestorage.app` default (committed in google-services.json) [apps/web/.env.example] — deferred: optional doc fix, out of 1.4 (`apps/mobile`-only) scope.
