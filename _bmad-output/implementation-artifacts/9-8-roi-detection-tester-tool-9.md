...
- [x] **Task 10: PR + sprint-status flip (AC: 13, 14)**
  - [x] Branch `tools-8-9-roi-discoverer-and-tester` cut off `main` (Stephane chose to bundle Stories 9.7 and 9.8 into a single branch/PR instead of the originally-hedged stacked-on-Tool-8 branch). Committed in 5 logical chunks (Tool 7 stats.npz amendment / Tool 8 deliverable / Tool 9 deliverable / wardentooling wiring / sprint-status + deferred-work docs), pushed, PR #10 opened against `main` (`feat(tooling): tools 8 & 9 — auto ROI discoverer + detection tester (Stories 9.7/9.8)`), merged 2026-05-14. Each commit subject lowercased to satisfy commitlint's `subject-case`.
  - [x] Flipped sprint-status `9-8-roi-detection-tester-tool-9`: `ready-for-dev → in-progress` → `review` (dev + tests + smoke done; flipped in one pass since the dev work was a single uninterrupted session) → `done` (this post-merge follow-up). `last_updated` bumped each time.
  - [x] Opened PR #10 (linked this story file + the Epic 9 charter + `apps/tooling/tools/auto_roi_discoverer.md` + the sibling Story 9.7 artifact); merged 2026-05-14.
  - [x] Post-merge follow-up — branch `stories-9.7-9.8-postmerge` (covers both 9.7 and 9.8 since they shipped in PR #10 together): flipped sprint-status `review → done`, bumped `last_updated`, flipped AC13/AC14 + these Task 10 sub-boxes to `[x]`, Status → `done` (Two-PR pattern per `feedback_two_pr_docs_execution.md`).
...
| Date       | Author                  | Summary |
|------------|-------------------------|---------|
| 2026-05-14 | post-merge follow-up (Opus 4.7 1M) | Two-PR post-merge follow-up (branch `stories-9.7-9.8-postmerge`, after main PR #10 merged to `main`). Status `review → done`; AC13/AC14 + Task 10's PR sub-boxes → `[x]`; sprint-status `9-8-roi-detection-tester-tool-9: review → done`, `last_updated` bumped, `epic-9` stays `in-progress` (9.1–9.4 still `backlog`). The follow-up covers Stories 9.7 and 9.8 together because Stephane chose to bundle them into a single branch (`tools-8-9-roi-discoverer-and-tester`, off `main`) and a single combined PR (#10, `feat(tooling): tools 8 & 9 — auto ROI discoverer + detection tester (Stories 9.7/9.8)`) instead of the originally-hedged stacked-on-Tool-8 branch. PR #10 carried five logical commits: (1) Tool 7 `stats.npz` side-car + circular-Hue stats (Story 9.7's Task-1 9.6 re-touch), (2) Tool 8 — auto ROI/HSV discoverer (Story 9.7's deliverable), (3) Tool 9 — per-frame ROI detection tester (this story's deliverable), (4) wardentooling Tools 8 & 9 menu wiring, (5) sprint-status + deferred-work docs. Each commit subject was lowercased to satisfy commitlint's `subject-case`. |
| 2026-05-14 | code-review (Opus 4.7 1M) | Joint `/bmad-code-review` pass on Stories 9.7 (Tool 8) + 9.8 (Tool 9) together (both tools uncommitted on the same working tree — reviewed in one shot). Three reviewers in parallel: Blind Hunter, Edge Case Hunter, Acceptance Auditor (**0 AC violations** across both stories — AC13/AC14 DEFERRED-OK per Two-PR pattern). After dedup/triage: 1 decision-needed (`_resize_to_ref` aspect-ratio handling for non-16:9 footage — Stephane chose option (b) warn-and-tolerate; converted to a patch), 22 patches applied total (13 Tool 8 + 8 Tool 9 + 1 decision-derived spanning both), 1 deferred (HSV bands fire on grayscale/white pixels — concept-level limitation, queued under `_bmad-output/implementation-artifacts/deferred-work.md`), 10 dismissed. Tool 9 HIGH fix: T9-H1 (lex sort would pick `v2.0` over `v10.0` once two-digit HUD versions exist — added `_version_sort_key` natural-sort helper). Tool 9 MEDIUM fixes: T9-M1 (`evaluate_frame` no longer wipes `map_scores={}` for non-map folders — preserves per-class scores for the CSV); T9-M2 (`_argmax_with_threshold` ties use `math.isclose` instead of exact float `==`); T9-M3 (`max_score==0` always returns `"unknown"` regardless of threshold — fixes the `--game-state-threshold 0.0` edge); T9-M4 (`iter_labeled_frames` `OSError` from `os.listdir` warns to stderr instead of silently returning empty). T9-M5 dismissed (false alarm — code already does the isinstance check before `data.get`). Tool 9 LOW fixes (2): T9-L1 `_resize_to_ref` `h<=0` guard; T9-L2 `_default_zones_path` per-entry try/except. Decision-resolution patch (option b): `main()` now reads `fragment.metadata["frame_shape"]` and emits one stderr warning if the first post-resize frame shape doesn't match (run continues — preserves experimental flexibility for non-16:9 footage testing). Regression-lock tests added (Tool 9 share): `_choose_version` v10>v2 natural sort, `_argmax_with_threshold` all-zero+threshold-0 returns unknown, `_version_sort_key` v10>v2, `_resize_to_ref` h=0 returns as-is, `evaluate_frame` preserves `map_scores` on non-map folders. Test suite 121 → 131 green (`cd apps/tooling && uv run pytest` + `pnpm --filter tooling test` both confirmed). Status stayed `review` — the `review → done` flip ships in the post-merge follow-up. |
