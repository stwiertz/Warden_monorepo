...
11. [ ] **AC11 — Sprint-status entry + lifecycle flip.** `_bmad-output/sprint-status.yaml` gains a `9-6-overlay-stack-analyzer-tool-7` entry under `epic-9`, added at create-story time and set to `ready-for-dev` (`epic-9` is already `in-progress`, so no epic flip — verify this happened before the dev agent reads this file). The entry then flows `ready-for-dev → in-progress → review` across the dev work, and `review → done` post-merge. _Held `[ ]` for the `review → done` portion — that flip is **post-merge admin** per the AC-checkbox-tighten convention; it ships in the tiny follow-up commit/PR after the main PR merges (Two-PR pattern, `feedback_two_pr_docs_execution.md`)._

12. [ ] **AC12 — Single-PR delivery + follow-up flip.** All file changes ship in **one PR** titled `feat: Tool 7 — overlay stack analyzer (Story 9.6)`, branch `tool-7-overlay-stack-analyzer`. PR body links: this story file; the Epic 9 charter at `_bmad-output/epics-and-stories.md` (with the note that Stories 9.5/9.6 were added post-planning via the new-HUD initiative and are not yet enumerated in the charter — known doc-debt, tracked, not corrected here); `apps/tooling/tools/video_timeline_labeler.md` (the upstream Tool 6 doc). The `review → done` sprint-status flip is a separate tiny follow-up commit/PR per AC11. _Held `[ ]` — post-merge admin._
...
- [ ] **Task 9: PR + sprint-status flip (AC: 11, 12)**
  - [ ] Branch `tool-7-overlay-stack-analyzer` created locally (off `tool-6-video-timeline-labeler`, since Tool 6 is not yet on `main` and Tool 7 depends on its directory contract / conftest / TUI registration). **Commit + push + PR pending the user's go-ahead** (`/bmad-dev-story` left the deliverable in the working tree on the branch). PR title when opened: `feat: Tool 7 — overlay stack analyzer (Story 9.6)` — should base on the Tool 6 PR (stacked) or wait for Tool 6 to merge.
  - [x] Flipped sprint-status `9-6-overlay-stack-analyzer-tool-7`: `ready-for-dev → in-progress` (work start) → `review` (dev work + tests + smoke done). `last_updated` bumped.
  - [ ] Open PR, link this story file + Epic 9 charter + `video_timeline_labeler.md`. _(Pending the commit/push go-ahead.)_
  - [ ] Post-merge: tiny follow-up commit/PR flips `review → done`, bumps `last_updated`, and flips AC11/AC12 + Task 9's PR boxes to `[x]` (Two-PR pattern per `feedback_two_pr_docs_execution.md`).
