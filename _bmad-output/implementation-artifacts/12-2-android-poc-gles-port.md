...
- [x] [Review][Decision] **🟠 MEDIUM — Path P's plane textures are fully re-specified every keyframe, so AC13's "4.7× cheaper" is partly an implementation artifact** — `WardenDetectionEngine.kt:466,482` call `glTexImage2D` (storage re-spec) rather than `glTexStorage2D` + `glTexSubImage2D`, for a 1920×1080 R8 plus a 960×540 RG8, on every one of 1061 keyframes. Every other texture in the class uses `glTexStorage2D` (`:163,:177,:215`), and `buildTargets()`'s own comment claims the plane textures are *"allocated **lazily on the first upload**"* — which the code does not do. Path P's forced-completion upload of **1.720 ms** therefore includes a driver-side realloc a steady state would not pay; that number is one half of AC13(a)'s *"4.7× cheaper, saving ~1.35 ms/keyframe"*. **Decide:** fix to `glTexSubImage2D` and re-measure (device time), or disclose the realloc as a caveat on the 1.720 ms.
  - **RESOLVED 2026-09-15 → patch, APPLIED, THEN REVERTED ON MEASUREMENT.** The patch converted `uploadYuvPlanes` to `glTexStorage2D` once + `glTexSubImage2D` per frame. **The device falsified its premise.** Full 1061-keyframe runs on the reference device: with the change, Path P upload **2.769 ms** and wall **50.682 ms**; reverted, **1.173 ms** and **42.652 ms** — against a delivered wall of 42.299. Re-specifying storage every frame is not waste, it is the **orphaning idiom**: the driver may hand back fresh backing and DMA into it without waiting for the previous draw to release the texture, whereas immutable storage forces `glTexSubImage2D` to synchronise against that pending read. An earlier A/B missed it because it ran cold and under the forced-completion profile, whose per-stage `glFinish` hides exactly this effect. Path Z was unaffected across every variant (bind 0.341–0.374 ms), which is what isolated the cause. **Reverted in `d70b306`; the measured rationale is recorded above `uploadYuvPlanes` so it is not "optimised" again.** AC13(a)'s delivered 1.720 ms needs no caveat — it was never inflated.
