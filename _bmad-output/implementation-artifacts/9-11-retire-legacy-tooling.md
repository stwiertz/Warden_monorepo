...
> AC1–AC7 are `[x]` (implemented + validated in the /bmad-dev-story 9.11 run). AC8 (sprint-status `review → done`) and AC9 (post-merge follow-up) remain `[ ]` and `[HELD]` per [[feedback_ac_checkbox_tighten]] + [[feedback_two_pr_docs_execution]] — their endpoints are post-merge admin, flipped by the `story-9-11-postmerge` follow-up.
