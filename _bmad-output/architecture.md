...
- **The process must stay alive at foreground importance for a multi-minute run.** This is the load-bearing reason, and it is engine-independent. A detection pass over a 1 h 14 capture costs ~35–45 s of compute today (measured, Story 12.2) and the surrounding auto-slice pipeline adds more; a backgrounded process is subject to cached-app freezing and low-memory kill long before that completes. A Foreground Service is the only Android mechanism that holds the process at foreground importance for that duration with a user-visible notification.
- **Thread affinity, not JS-context affinity, is the engine's real constraint.** Story 12.2 measured the threading model directly: the engine runs on a plain worker thread it owns (`warden-bench`) — never the UI thread — with `MediaCodec` callbacks handled on that same thread. **The engine needs *a* thread it owns, not the main JS context.** A Foreground Service gives it a host process; the thread is its own.
- `expo-task-manager` remains rejected, on a narrower argument than before: it runs JS in a **separate headless context** with its own lifecycle, so pipeline orchestration state (`processingPipeline.ts`'s stage machine, the MMKV checkpoints, the progress the notification renders) would have to be marshalled across a context boundary Android may tear down independently. That is fragile for reasons of **process and context lifecycle**, not because a native binding cannot cross it.

> **RATIONALE RE-DERIVED 2026-09-15 (Story 12.3) — amendment 5d RESOLVED; the `RATIONALE RE-DERIVATION REQUIRED` banner is removed.** The decision (custom `expo-config-plugin`) was never reversed; **its rationale was, and the bullets above are the replacement.** The previous bullets rested entirely on **JSI-context affinity** — *"the FFmpeg/OpenCV JSI bindings cannot be shared across the headless context boundary"*, and below, *"the service hosts the main JS context where the JSI binding lives"*. Story 12.2 measured the real model and Story 12.3 wrote it up: **EGL contexts are THREAD-bound, not JS-context-bound**, and the engine bound by **Decision #13** has no EGL context at all.
>
> **What changed and what did not.** The conclusion survives — a Foreground Service is still the right host — but **because the process must stay alive at foreground importance for a multi-minute run, not because the JS context must be co-located.** Under Decision #13 (CPU rule evaluation on MediaCodec keyframe decode) the thread-affinity argument **narrows further**: with no EGL context there is no GL thread-binding constraint at all, only MediaCodec's own callback thread. **The process-lifetime argument is unchanged and is now the sole load-bearing one.**
>
> Note the old bullet's warning that cross-context work *"would require a fundamental redesign of the processing pipeline"* — **that redesign is Story 12.4's**, and it is now scoped by a bound engine rather than an open question. Measured model: `apps/mobile/bench/12-2/REPORT.md` §8 and `device_report_timing.json` → `ac16_threading_model`. Verdict: [`architecture-spike-gpu-megashader.md`](architecture-spike-gpu-megashader.md). See also [`sprint-change-proposal-2026-07-16.md`](sprint-change-proposal-2026-07-16.md).

- Custom `expo-config-plugin` injects native Android service-launching code into the prebuild output (`AndroidManifest.xml` foreground-service permissions + a foreground service class). The service **holds the process at foreground importance** for the processing duration, with a sticky notification. *(Amended 2026-09-15, amendment 5d: this bullet read "hosts the main JS context" — the service hosts the **process**; the engine owns its own worker thread.)*
...
>
> ### ➡️ FORWARD POINTER — the engine question is CLOSED (2026-09-15, Story 12.3)
>
> **Epic 12 has reported. The verdict is [`architecture-spike-gpu-megashader.md`](architecture-spike-gpu-megashader.md), recorded architecturally as [Decision #13](#decision-13--detection-engine-rule-evaluation-arm-cpu-not-gpu-resolved): the GPU mega-shader is REJECTED; CPU rule evaluation on the MediaCodec keyframe-decode path is bound.** PERF-002 is re-baselined there from reference-device measurement (≤ 1.02% of source duration against a 5% budget). PERF-010 **stays a soft target** — the run measured one device, and a minimum-supported-device floor is a claim about the device population.
>
> **Everything below this banner, and Story 1.1's rung-0 verdict in [`architecture-spike-perf-floor.md`](architecture-spike-perf-floor.md), is FROZEN audit trail.** **12.3 supersedes; 1.1 records.** Do not edit the record to match the new verdict — read the forward pointer instead. *(FROZEN-block discipline, per the 2026-05-09 binding-only cut and Story 9.10's convention.)*
...
5. **Validate accuracy floors:**
   - Map ID accuracy ≥ 95% on an unseen test set. **The instrument is Tool 9 (`apps/tooling/tools/roi_detection_tester.py`), re-pointed at the bound engine by Story 9.16 — NOT `hash_validator.py`.** *(Amendment 5f completed 2026-09-15, Story 12.3. This line named `hash_validator.py`, which measures **Hamming distance between perceptual hashes** and cannot measure a zone/HSV-scoring engine at all. The 2026-07-16 pass re-pointed only one of this amendment's three sites; this is the second. **Without Story 9.16 this floor has no instrument.**)*
...
|---------|-------------|-------------|
| **Pass** — all 4 PERF NFRs met on reference device with a real engine binding *(engine-agnostic per 2026-07-16; was "with real JSI binding")* | None — V1 launches with auto-slice on | **VERDICT 2026-09-15 (Story 12.3): `rung-0 PROVISIONAL pending Story 12.4`.** Only **PERF-002** is measured, and it is **MET** — re-baselined at **≤ 1.02% of source duration** (44.879 s against a 220.98 s budget; see [`architecture-spike-gpu-megashader.md`](architecture-spike-gpu-megashader.md)). **PERF-003/PERF-004 are un-instrumented** (rung 2) and **PERF-005 is unmeasured** (the export surface does not exist). And *"with a real engine binding"* is **not yet true** — Story 12.2 shipped a **bench**, not a pipeline binding; the engine is chosen, not wired. *(Corrected 2026-09-15: this cell previously said "PRD inherits the measured PERF-010 number" — **PERF-010 remains a SOFT TARGET** and is not bound by a single-device run. The round-boundary accuracy floor is likewise not bound here: REL-006's instrument is Tool 9 via Story 9.16.)* |
| **Partial fail** — PERF-002 or PERF-005 over budget by < 50% | **Rung 1 — RE-ARMED 2026-09-15 (Story 12.3), two steps, cheapest first:** **(1a)** reduce per-keyframe **decode-loop overhead** — shorten `TIMEOUT_US` (~10.6 ms/kf) and stop flushing per keyframe by pipelining IDRs (~14.5 ms/kf); **(1b)** **decimate keyframes** — process every *N*th keyframe. | **The previous remedy (*"lower auto-slice frame-sampling rate"*) was INERT under a keyframe-only engine and had never been recorded as such** — the sampling rate **IS** the GOP (4.1667 s / 1061 keyframes), there is no continuous frame-rate knob, and rule evaluation is only **6.0–7.3% of the wall**, so the cost it reached for is not where the time goes. *(Same defect class as rung 3's unreachable "JSI binding does not ship" trigger, fixed 2026-07-16 while this rung was left alone.)* **Step 1a engages the measured cost — 25.1 of 42.2 ms/keyframe — at no accuracy or granularity cost; try it first.** Step 1b is what *"lower the sampling rate"* can honestly mean here, and it costs detection granularity (4.17 s → *N* × 4.17 s). Re-measure; if pass, V1 launches with the applied step documented as an architectural constraint. PRD `mobile-AUTO-SLICE-001`'s *"with reduced sampling on weak hardware"* clause attaches to **step 1b** and survives intact; **step 1a carries no PRD clause** because it has no user-visible effect. |
| **Partial fail** — PERF-003 or PERF-004 over budget — 🔴 **TRIGGER UN-INSTRUMENTED (2026-09-15, Story 12.3)** | Rung 2: drop Minimap+HUD overlay rendering on weak hardware (gated by device profile) | **The remedy is sound; the trigger cannot fire from measurement.** PERF-003 (view-mode toggle ≤ 100 ms) and PERF-004 (Cinema Mode cold-start ≤ 1.5 s) have **never been measured** — Story 1.1's substrate-gap audit found the surfaces do not exist (no `expo-av`/`expo-video`, no `<Video>` component, `CinemaModeScreen.tsx` a visual stub, no Cinema/Card routes) — and both became **soft UX targets on 2026-05-09** under the ship-and-observe decision. **Rung RETAINED, not retired** (the remedy is the right one and should exist the day a number does) and **not re-armed** (arming it means building a view-mode / cold-start timing instrument, which no story currently owns). Until then it fires on **observed** behaviour, not on a measurement. Arming it belongs with the surfaces — **Stories 5.4 / 5.5 / 6.6**. View modes degrade to Full + Minimap (no HUD overlay). PRD `mobile-CINEMA-002` adds a graceful-degradation clause: device-profile-gated. |
| **Hard fail** — the on-device detection engine does not ship as a real binding within V1 timeline *(re-armed 2026-07-16: this rung previously triggered only on "JSI binding does not ship", which is **unreachable** under a GPU-shader engine — the trigger is now engine-agnostic and fires on whichever engine Story 12.3 binds)* | Rung 3: defer auto-slice to V2; V1 ships manual-clip-only | **VERDICT 2026-09-15 (Story 12.3): DOES NOT FIRE — but NOT YET RETIRED.** An engine **is** bound (Decision #13: CPU rule evaluation on MediaCodec keyframe decode), both arms are written, the bound one is validated at **0 fire-bit disagreements over 357,244 decisions**, and a working decode path exists on the reference device (count-asserted 1061 == 1061). It is not retired because the **binding** — wiring the engine into `processingPipeline.ts` — is **Story 12.4's**, and this trigger is about shipping a real binding *within the V1 timeline*. **Rung 3 retires when Story 12.4 lands;** if 12.4 does not land in the V1 timeline it fires on its own terms. Then: `mobile-AUTO-SLICE-*` FRs become V2; `mobile-CARD-*` graceful-degradation: card view hides auto-sliced rounds, exposes timeline-only manual-clip flow. PRD activation timer T1-coach via manual-clip path becomes the only path. **V1 launches.** |
...
> ### 🔴 LADDER AUDIT — 2026-09-15 (Story 12.3, engine verdict)
>
> All five rows were audited against **"is the trigger reachable, AND does the remedy engage the measured cost?"** Result: **Pass row** provisional pending Story 12.4 · **rung 1 RE-ARMED** (its remedy was inert) · **rung 2 trigger labelled UN-INSTRUMENTED** · **rung 3 answered** (does not fire; retires with 12.4) · **FORBIDDEN row carried VERBATIM and unchanged.**
>
> The full audit, with the measured basis for every row, is in [`architecture-spike-gpu-megashader.md`](architecture-spike-gpu-megashader.md) → *Ladder rung verdict*. The engine bound is **Decision #13** — CPU rule evaluation on the MediaCodec keyframe-decode path; the GPU mega-shader is **rejected on measurement** (6.4× slower than the device CPU, bit-identical output, ~6% of the wall either way).
>
> **The ladder is now armed for the first time since the pivot.** Before this audit, rung 1's remedy could not engage the measured cost and rung 2's trigger had no instrument — two of five rows read armed and were not.
...
#### Decision #13 — Detection-engine rule-evaluation arm: CPU, not GPU [RESOLVED]

*Added 2026-09-15 by Story 12.3 (amendment 5g — `#### Implementation Handoff` states the decision entry lands with that story). This is the **fifth native module's** authorizing decision. Full evidence and the ladder audit: [`architecture-spike-gpu-megashader.md`](architecture-spike-gpu-megashader.md).*

**Choice:** **(a) REJECT the GPU mega-shader. Bind Kotlin CPU rule evaluation on the MediaCodec keyframe-decode path.**

What is bound: MediaCodec keyframe decode via absolute `SEEK_TO_CLOSEST_SYNC` per keyframe, ByteBuffer output with bit-parity YUV→RGB conversion; rule evaluation by the integer `RGB2HSV_b` Kotlin arm (`WardenCpuBaseline`); rule packing by `WardenRulePacker`, byte-identical to `lut.py`; the keyframe index taken from the seek-built PTS list.

What is dropped: the EGL context and pbuffer surface, the GLES mega-shader, the LUT texture upload, the full-frame resolve pass, `glReadPixels` synchronisation, driver-defined OES colour conversion, and the entire Path Z / Path P colour fork.

**Rationale:**

- **The GPU is 6.4× SLOWER at the work it was introduced to accelerate.** Reference device, decode excluded from both sides, 400 frames × 134 rules: **CPU 0.376 ms/frame vs GPU 2.389 ms/frame (0.157×)**. Composing the GPU arm out of Path Z's real forced-completion stages instead gives **2.577 ms = 6.86×** — the conclusion is robust to how the arm is built, and it is *worse* than the PC's 0.51×.
- **The pivot's founding premise is falsified, and that is the most valuable thing Epic 12 produced.** The chain was *PERF-002 is unacceptable → per-frame CPU rule evaluation is the ceiling → a GPU mega-shader removes it.* Link 2 is false: **rule evaluation is 6.0–7.3% of the wall on the GPU and ~1% on the CPU.** Choosing CPU over GPU moves a 35–45 s run by **~2.7 s**. The real cost is a per-keyframe pipeline teardown-and-restart — `flush()` 14.5 ms plus 10.6 ms sleeping in a `TIMEOUT_US` we chose — **~25 ms/keyframe that is ours to fix and has nothing to do with the engine** (Story 12.4).
- **Correctness is explicitly NOT the reason.** The shader is bit-exact on Adreno 642L: **0 disagreements across 357,244 rule-frame decisions**, including the 31 rules with bands asymmetric about their centre, the 9 wrap-branch rules that depend on negative signed `>>` sign-extending, and the 69 low-saturation rules. **If this decision were about correctness the GPU would survive it.** The two arms are indistinguishable in output; they differ only in cost.
- **The GPU's cost is structural, not tuning-fixable.** The shipped rects are 1–25 px (fourteen are 1×1) — ~800 texel fetches per frame — against fixed per-draw costs of ~2.58 ms (resolve 1.306 + shader 0.598 + readback 0.305 + bind 0.367). **The arithmetic was never the bottleneck, so making it parallel buys nothing.** Only a very much larger ruleset would change that ratio, and none is proposed: the ruleset is 134 rules across 13 maps, growing by one rule set per new map.
- **Rejecting it removes an architectural surface at no measured cost** — no EGL context to own and make current, no GL thread-affinity constraint, no LUT texture upload, no readback synchronisation, and no **driver-defined, AOSP-unspecified** colour conversion. Story 12.2 lost a full parity run to one such failure: a rule-major vs row-major transpose that produced plausible-looking detections from garbage, which both the LUT byte-check and the render-target self-test passed straight through.

**Alternatives rejected:**

- **(b) Bind the GPU mega-shader anyway.** Defensible only on a **non-performance** argument — headroom against a much larger future ruleset, or offloading the CPU for concurrent work. Neither is on the roadmap, and no such justification was offered. Rejected **on measurement**: it costs 6.4–6.9× more for bit-identical output, and activates a permanent Android-only GLES surface.
- **(c) Bind the decode architecture now, defer the rule-eval arm to Story 12.4.** Rejected because it re-creates the defect this decision exists to close: Story 9.16 cannot re-point Tool 9 at an engine that is not chosen, leaving **REL-006's ≥ 95% floor with no instrument at all**. And there was almost nothing left to defer — both arms are written, and the bound one is already validated at 0 disagreements.
- **The CPU arm and the GLES mega-shader are each other's rejected alternative, and the rejection is on measurement**, not on preference, portability or taste.

**Implementation:**

- `apps/mobile/plugins/with-detection-engine.js` — the Expo config plugin that emits the Kotlin sources at prebuild. **Narrows** to MediaCodec + CPU rule evaluation; the GLES/EGL sources are removed by **Story 12.4**, not here.
- Sole JS access via `apps/mobile/src/shared/services/detectionEngine.ts`, per [INVARIANT: native-modules-only-via-shared-services](#mobile-native-modules). The engine is the **fifth** native module and is listed in the [SEC-007 allowlist](#sec-007-third-party-sdk-allowlist) (entry 5).
- `WardenCpuBaseline` (integer `RGB2HSV_b`) and `WardenRulePacker` are already written and validated; Story 12.4 wires them into `processingPipeline.ts` and the four detection modules.
- **`countSyncSamplesByScan()` must never ship** — it is a full-file read costing 62.070 s. Take the index from the seek-built PTS list (2.290 s) or read `stss` directly.

**Cascading implications:**

- **PERF-002 is re-baselined from measurement** — ≤ 1.02% of source duration on the reference device against a 5% budget (`prd.md`). The re-baseline covers the **detection pass only**; segmentation, thumbnail export and the rest of the auto-slice pipeline were never measured, and Story 12.4 must re-measure end to end.
- **PERF-010 stays a soft target.** A single-device run cannot bind a minimum-supported-device floor.
- **The Innovation #1 fallback ladder is re-armed** against this engine — rung 1's remedy replaced (it was inert under a keyframe-only engine), rung 2's trigger labelled un-instrumented, rung 3 answered, the FORBIDDEN row unchanged. Rung-0 is **provisional pending Story 12.4**.
- **Amendment 5c re-scopes rather than lapses** — MediaCodec keeps decode Android-only even with no GLES.
- **Amendment 5d's rationale is re-derived** — the Foreground Service is justified by process lifetime, not JS-context co-location.
- **Story 9.16** re-points Tool 9 and `video_test.py` at **this** engine; without it REL-006 has no instrument. **Story 12.4** wires the engine into the pipeline, removes the GLES surface, and takes the decode-loop fixes.
- **[INVARIANT 3] is untouched** — the bound engine is on-device by construction. Cloud-fallback CV remains FORBIDDEN.
...
> **AMENDED 2026-07-16; RESOLVED 2026-09-15 (Story 12.3) — amendment 5c is RE-SCOPED, not lapsed.** **The assertion above is FALSE for the detection-engine slice** and is retained only as the pre-pivot record.
>
> **Why it does not lapse even though the shader was rejected.** This amendment previously said it *"lapses and the original assertion stands unmodified"* if Story 12.3 rejected the shader engine. **It does not lapse.** Story 12.3 rejected the GLES mega-shader (**Decision #13**) but **bound MediaCodec keyframe decode**, and `android.media.MediaCodec` / `MediaExtractor` are **Android platform APIs with no iOS equivalent** — the iOS counterpart is VideoToolbox, a different API with different semantics for keyframe seek and output buffering. **Decode is Android-only by construction, with or without GLES.** The amendment is therefore **re-scoped from "decode + GPU compute" to "decode only"**, not reverted.
>
> **What the rejection does buy back:** the GLES/EGL half of the port disappears. Any future iOS Phase-2 estimate must still carry a **decode port** line item (MediaCodec → VideoToolbox) but **no longer a GPU-compute port** — the bound rule evaluator is a plain integer loop over pixels, which ports to any platform as ordinary code. **The expensive half of the Android-only surface went away; the cheaper half remains.**
>
> **Cost, stated rather than hidden:** iOS is already V3 in the PRD, so the cost is deferred, not incurred. Every *other* mobile decision (codebase, data layer, MMKV/SQLite, Foreground Service plugin) remains cross-platform-ready — the Android-only surface is bounded to **decode**. **iOS Phase 2 is glue everywhere except the decode path, where it is a port.** See [`architecture-spike-gpu-megashader.md`](architecture-spike-gpu-megashader.md) and [`sprint-change-proposal-2026-07-16.md`](sprint-change-proposal-2026-07-16.md).
...
- **Pre-PRD performance spike outcome** dictates whether the auto-slice FRs ship in V1 or defer to V2 (Innovation #1 ladder rung 3). *(Amended 2026-09-15, Story 12.3: this bullet said the spike **binds PERF-010** — it does not. **PERF-010 remains a soft target**, and the Epic 12 spike re-pointed which spike would report a number without re-arming a measured floor. What the spike did bind is the **engine** ([Decision #13](#decision-13--detection-engine-rule-evaluation-arm-cpu-not-gpu-resolved)) and the **PERF-002 re-baseline**. Ladder rung 3 **does not fire** but retires only when Story 12.4 lands the pipeline binding.)*
...
│   ├── architecture-spike-gpu-megashader.md                 [NEW] Epic 12 engine spike verdict (Story 12.3 — binds Decision #13; re-baselines PERF-002)
...
All decisions land in a compatible dependency order. The coordinated triples (Decision #1/#6/#9, Decision #2/#7/#8) close together; Brownfield Item 5 (Firebase v12 RN auth migration) is sequencing-blocking for the entitlement state-machine end-to-end test but compatible with Decision #3 (shared Firebase project — `@react-native-firebase` reads project ID from `google-services.json`, which encodes the same project ID as the web client config). The Foreground Service custom config plugin (Brownfield Item 6) is compatible with the bound detection engine (**Decision #13**): the foreground service **holds the process at foreground importance** for a multi-minute run, while the engine owns its own worker thread — avoiding the process- and context-lifecycle issues `expo-task-manager` would introduce. *(Amended 2026-09-15, amendment 5d. This sentence read "the foreground service hosts the main JS context where the JSI binding lives"; Story 12.2 measured the threading model and established that **EGL contexts are thread-bound, not JS-context-bound**, and Decision #13's engine has no EGL context at all. The conclusion is unchanged; the reason is corrected.)*
...
| 5 | **MediaCodec detection engine** (`team.warden.mobile.Warden*`) — *GLES/EGL REJECTED 2026-09-15, see below* | **Story 12.2** | **NOT a third-party SDK** — a first-party Kotlin wrapper over AOSP **platform** APIs. **Bound surface (Decision #13): `android.media.MediaCodec`, `android.media.MediaExtractor`** plus a plain Kotlin integer rule evaluator. **`android.opengl.GLES30`/`EGL14`/`SurfaceTexture` are rejected architecturally and are removed by Story 12.4** — they are still present in the emitted sources today, which is why this entry names them as pending removal rather than omitting them. Emitted by `apps/mobile/plugins/with-detection-engine.js`; sole JS access via `apps/mobile/src/shared/services/detectionEngine.ts` ([INVARIANT: native-modules-only-via-shared-services](#mobile-native-modules)). Listed because the invariant above requires the entry, not because a vendor SDK was introduced. | **None.** No socket, no HTTP client, no telemetry. Reads a local capture and a local `map_config`; writes JSON to the app's own `getExternalFilesDir`. |
...
> **Status of entry 5 — RESOLVED 2026-09-15 (Story 12.3).** This entry recorded the dependency-level facts Story 12.2 measured, and explicitly did **not** authorize the GPU engine architecturally. **Story 12.3 has now decided: [Decision #13](#decision-13--detection-engine-rule-evaluation-arm-cpu-not-gpu-resolved) REJECTS the GPU mega-shader and binds MediaCodec keyframe decode + Kotlin CPU rule evaluation.**
>
> **The engine survives in reduced form, so entry 5 is CORRECTED rather than deleted.** The module still exists, still obeys the native-modules-only-via-shared-services invariant, and still belongs on this allowlist — it has simply lost its GLES/EGL half. Entry 5's text is amended above accordingly. **The physical removal of the GLES/EGL sources from `apps/mobile/plugins/**` and the `Warden*.kt` files is Story 12.4's, not this story's** — the decision is recorded here; the code is left alone.
>
> **Entry 5a re-evaluated and RETAINED for now.** `ffmpeg-kit-main-16kb` was re-declared purely for compile-time visibility of `com.arthenica.ffmpegkit` from our Kotlin, to build **AC0b Option C's software-decode control**. That control was a bench instrument, and **Decision #13 binds MediaCodec, not ffmpeg-kit**, so the compile-time coordinate has no consumer under the bound engine. It is **not removed here** because nothing new ships either way — the same coordinate and version already reach the APK transitively via entry 1, and there is no version to resolve against. **Story 12.4 drops the re-declaration when it removes the bench surface**; if it turns out to have a consumer, this entry stays as written.
>
> *(Entry added 2026-09-15 during Story 12.2's code review — AC18(c) required it and it had been closed by prose alone. Status resolved the same day by Story 12.3.)*
...
- **Performance (10/10):** PERF-001 (activation timer ≤300s) covered by `cross-ACTIVATION-001` telemetry contract. PERF-002/003/004/005 (mobile timing budgets) covered by spike + "no player swap" view-mode pattern + 4-stage MMKV-checkpointed pipeline. PERF-006/007 (web LCP/FCP/TTI) already met by current implementation; architecture inherits regression-coverage requirement. PERF-008 (webhook ≤1s p95) already met. PERF-009 (tooling linear scaling) covered by reference implementation. **PERF-010 (mobile reference-device floor): SOFT TARGET, not a measured floor — disposition recorded 2026-09-15 by Story 12.3** ([`architecture-spike-gpu-megashader.md`](architecture-spike-gpu-megashader.md)). The 2026-05-09 ship-and-observe decision softened this from "measured floor TBD", and Epic 12's reference-device run does **not** bind a number here: it measured detection-pass throughput on **one** device, and a minimum-supported-device floor is a claim about the device *population*. V1 launch is not gated on it. **PERF-002, by contrast, IS now re-baselined from measurement** — ≤ 1.02% of source duration on the reference device, against a 5% budget.
...
- **Reliability (6/6):** REL-001 covered by AUTOSAVE FRs + MMKV checkpoint pattern. REL-002 (30-day offline) covered by entitlement state machine `offline-grace` state + Decision #2 hybrid `map_config.json`. REL-003 (webhook delivery delay tolerance) covered by `stripe_events/` manual-replay path. REL-004 (entitlement transition latency) covered by webhook → Firestore → mobile foreground re-fetch cycle. REL-005 (tooling determinism) covered by stateless-pure-function pattern. REL-006 (accuracy floors) covered by **Tool 9 (`apps/tooling/tools/roi_detection_tester.py`), re-pointed at the bound engine by Story 9.16** + `tooling-VALIDATE-001`. *(Amendment 5f completed 2026-09-15, Story 12.3 — third and last site. `hash_validator.py` measures Hamming distance between perceptual hashes and cannot measure the bound zone/HSV engine; it was named here in error. **Until Story 9.16 lands, this gate has no instrument.**)*
...
1. **Pre-PRD performance spike** (architecture-led; gates everything else; binds the Innovation #1 ladder rung). Deliverables: [`architecture-spike-perf-floor.md`](architecture-spike-perf-floor.md) (Story 1.1 — JSI binding viability, **FROZEN record**) and [`architecture-spike-gpu-megashader.md`](architecture-spike-gpu-megashader.md) (**Story 12.3 — the engine verdict, [Decision #13](#decision-13--detection-engine-rule-evaluation-arm-cpu-not-gpu-resolved), and the PERF-002 re-baseline; supersedes for the engine question**). *(Amended 2026-09-15: "binds PERF-010" removed — PERF-010 is a soft target, not a measured floor.)*
