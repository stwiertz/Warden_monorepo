...
- **The on-device detection engine ships as a real binding** (not the current tested-via-injection stub). Map identification + auto-slicing FRs depend on this. **Which engine — CPU/JSI vs. the GPU mega-shader under evaluation — is bound by Story 12.3 (Epic 12, V1-GATING).** *(Amended 2026-07-16; previously named the OpenCV JSI binding. Note the Innovation #1 fallback ladder was **re-armed** at the same time: its hard-fail rung triggered only on "JSI binding does not ship", which is unreachable under a shader engine — leaving the auto-slice FRs with no V1 safety net until Story 12.3 lands.)*
...
- **PERF-002 (mobile):** Auto-slice processing time ≤ 5% of source video duration on the architecture-bound reference device (e.g., 1h20 source → ≤ 4 minutes auto-slice). The brief's J1 implies ~90s for 1h20 = ~2%; the 5% floor sets V1 acceptance with reference-device-specific tuning by architecture. **RE-BASELINE PENDING — amended 2026-07-16 (correct-course, engine-first pivot):** this budget is one of the three triggers for the Epic 12 pivot (4 minutes for a 1h20 session was judged unacceptable UX, and the per-frame CPU path was judged to be the ceiling rather than its tuning). **Story 12.3 re-baselines this number from Story 12.2's measured results on the Poco X5 Pro 5G.** The engine brief's **< 10 s** for the same input is recorded as the **aspiration, not an acceptance criterion** — the POC measures and reports; it is not failed for missing it. Consistent with the 2026-05-09 ship-and-observe decision that already downgraded the numeric PERF budgets to soft UX targets. **Only a reference-device run can bind this; a PC POC number is never a mobile number.**
