...
> ## AMENDED 2026-07-16 — Engine-First Pivot (detection mechanism de-mechanized)
>
> Stephane, `/bmad-correct-course`. Full rationale: [`sprint-change-proposal-2026-07-16.md`](sprint-change-proposal-2026-07-16.md).
>
> **What changed:** this PRD names **"the OpenCV JSI binding"** as the load-bearing V1 milestone in roughly a dozen places. That is a *mechanism*, and it is now stale — a GPU mega-shader engine (keyframe-only FFmpeg decode -> GLSL rule bank -> CPU scoring) is under evaluation as its replacement, gated by **Epic 12** and bound by **Story 12.3**. Everywhere this document says "the OpenCV JSI binding", read **"the on-device detection engine (bound by Story 12.3)"**. The load-bearing *outcome* — a real on-device engine, not a stub — is unchanged and still V1-blocking. The most load-bearing occurrences are amended inline below; **Story 9.10 (scope widened) owns the exhaustive prose sweep.**
>
> **What did NOT change — and this is the point:**
> - **MVP scope, personas, journeys, the six-state entitlement machine, business targets, and pricing are all untouched.** This is a mechanism change, not an MVP change.
> - **Innovation #3's privacy + marginal-cost claim survives intact.** A GPU shader is on-device: no video crosses the wire, per-match infrastructure cost stays zero. Cloud-fallback CV remains **FORBIDDEN** (architecture `:858`, [INVARIANT 3]).
> - **Accuracy floors are unchanged** (REL-006: map ID ≥ 95% on unseen). Only the *instrument* changes — see the REL-006 note in Technical Success.
> - **`mobile-AUTO-SLICE-003`'s `unknown` fallback is unchanged**, and it is where the new engine's **"doubt" state** lands: a phase that clears no confidence threshold is reported as unknown rather than forced to the nearest class. Below-floor behaviour remains graceful degradation, not a blocking error.
>
> **Sequencing note (engine-first).** Epics 5/6/7 (Card View + Cinema Mode, clips/voice/export, auto-save) are **held** behind Story 12.3 + Story 12.4 (the mobile consumer rewrite). This is a **development-focus decision, not a dependency-driven one** — the app supports manual clips and J3 explicitly designs for degradation. It is recorded plainly so a future reader who checks the dependency graph is not misled.
...
3. **On-device privacy + structurally lower marginal cost.** No video leaves the phone. The on-device pipeline (CPU-bound — FFmpeg, OpenCV via JSI, pHash) carries zero per-match infrastructure cost; fixed costs (Firebase reads/writes, Stripe %) scale linearly with paying users, not footage volume. Cloud-pipeline competitors absorb per-match storage + GPU-vision cost on every uploaded clip — at €7.99/mo their margin compresses with every active session; Warden's does not. **The on-device detection engine is the load-bearing V1 milestone** (today a tested-via-injection stub; the engine itself is bound by **Story 12.3** — amended 2026-07-16, this previously named the OpenCV JSI binding specifically). Functional requirements that depend on auto-slicing or on-device map identification inherit this risk and are flagged in their FR rows. *Note the pipeline description above says "CPU-bound" — under the Epic 12 engine the per-frame work moves to the GPU, but the claim that matters here is unaffected: it is still on-device, still zero per-match infrastructure cost.*
...
- **On-device-only video processing** — privacy, offline-capable, structurally lower marginal cost. The **on-device detection engine** shipping as a real binding (not the current tested-via-injection stub) is V1-load-bearing; **which** engine is bound by Story 12.3 (Epic 12). *(Amended 2026-07-16 — previously named the OpenCV JSI binding. The commitment is to on-device processing, not to a particular binding technology.)*
...
- **Detection-engine dependency:** auto-slicing is the load-bearing path between T0 and T1. If the engine ships as the current tested-via-injection stub rather than a real binding, T1 fires only on the manual-clip path. Treated as a V1 launch blocker. *(Amended 2026-07-16 — previously "OpenCV JSI binding dependency"; the engine is bound by Story 12.3.)*
...
- **The on-device detection engine ships as a real binding** (not the current tested-via-injection stub). Map identification + auto-slicing FRs depend on this. **BOUND 2026-09-15 by Story 12.3 (Epic 12): the GPU mega-shader is REJECTED; the engine is Kotlin CPU rule evaluation on the MediaCodec keyframe-decode path** ([Decision #13](architecture.md); evidence in [`architecture-spike-gpu-megashader.md`](architecture-spike-gpu-megashader.md)). The GPU measured **6.4× slower** than the device CPU at rule evaluation for **bit-identical** output, and the engine is only **6–7% of the wall** either way. **The milestone is not yet met:** the engine is *chosen*, not *wired* — Story 12.2 delivered a bench, and **Story 12.4 ships the real binding into `processingPipeline.ts`.** *(Amended 2026-07-16, resolved 2026-09-15; previously named the OpenCV JSI binding. The Innovation #1 fallback ladder was **re-armed** in 2026-07-16 for its hard-fail rung only — that rung had triggered on "JSI binding does not ship", unreachable under a shader engine. **Story 12.3 completed the re-arm across all five rungs**, including rung 1, whose "lower the frame-sampling rate" remedy was **inert** under a keyframe-only engine, and rung 2, whose trigger is now explicitly labelled **un-instrumented**. The auto-slice FRs have a working V1 safety net again.)*
...
|---|-----------|-----------------|---------------|-------------|
| 1 | On-device CV pipeline | Performance floor met on reference device per architecture spike (Risk #2 escalation); accuracy floors met (≥95% map ID, round-boundary TBD per Section 2); the detection engine ships as a real binding | Reference-device performance below floor; accuracy below floor on unseen test set; the detection engine fails to ship as a real binding (V1 launch blocker) | Architecture-published performance number on reference device; **Tool 9 (`roi_detection_tester.py`) accuracy report — amended 2026-07-16, was `hash_validator` regression suite, which measures Hamming distance between perceptual hashes and cannot measure a zone/HSV-scoring engine at all (see Story 9.16, which re-points Tool 9 at the engine Story 12.3 binds; without it this gate has no instrument)**; CI test for engine-binding-not-stub |
...
- **mobile-AUTO-SLICE-001:** Mobile auto-slices an imported session into per-round Cards using on-device round-boundary detection. *(J1, J6) (load-bearing on the on-device detection engine shipping as a real binding — bound by Story 12.3; amended 2026-07-16)*
- **mobile-AUTO-SLICE-002:** Mobile auto-identifies the map for each round using **on-device map identification** against `map_config`. *(J1, J6) (load-bearing on the on-device detection engine — bound by Story 12.3)* **Amended 2026-07-16:** this FR's text said "perceptual hashing" — a *mechanism* baked into a requirement. It was already stale from the 2026-05-14 ROI+HSV pivot and stale again under Epic 12. The FR now names the outcome; the mechanism lives in the architecture where it can change without a requirements edit.
...
- **PERF-002 (mobile):** Auto-slice processing time ≤ 5% of source video duration on the architecture-bound reference device (e.g., 1h20 source → ≤ 4 minutes auto-slice). The brief's J1 implies ~90s for 1h20 = ~2%; the 5% floor sets V1 acceptance with reference-device-specific tuning by architecture. **RE-BASELINED 2026-09-15 (Story 12.3) — the NFR is MET, by ~4.9×.** Measured on the reference device (Poco X5 Pro 5G / SM7325 / Adreno 642L / Android 14), on a **4419.633 s** capture (1 h 13 min 40 s — *not* the "~2 h" the Epic 12 story text said):

  | | value |
  | --- | --- |
  | source duration | **4419.633 s** |
  | budget @ ≤ 5% | **220.98 s** |
  | **measured, bound configuration (upper bound)** | **44.879 s = 1.02% of source duration** (~×98 real time) |
  | *(measured, zero-copy GPU path — the configuration Story 12.3 REJECTED)* | *35.467 s = 0.80%* |

  **🔴 Read what is inside this number.** (1) It is a **detection-pass bench, not an auto-slice run** — it **excludes segmentation, thumbnail export and the rest of the auto-slice pipeline this NFR actually scopes**, all of which are **unmeasured**; the one-off keyframe index (2.29 s by seek) is reported outside the wall. The margin is wide enough (~176 s of budget unused) that the excluded stages would have to cost ~4× the entire detection pass to breach it, but **Story 12.4 must re-measure PERF-002 end to end over the real pipeline before this NFR is treated as closed.** (2) The bound figure is the **bit-parity ByteBuffer path**, not the faster zero-copy path — zero-copy decodes into a GL texture to feed a shader, and **Story 12.3 rejected the shader** ([Decision #13](architecture.md)); a CPU rule evaluator needs the pixels on the CPU.

  **The `< 10 s` aspiration is MISSED and is NOT an acceptance criterion** — it is the engine brief's aspiration; the POC measures and reports and is **not failed** for missing it, consistent with the 2026-05-09 ship-and-observe decision that downgraded the numeric PERF budgets to soft UX targets. **It is also not the engine's fault:** rule evaluation is only **6–7% of the wall**, and ~25 ms of every 42 ms keyframe is per-keyframe pipeline teardown plus a dequeue timeout we chose ourselves — **~26 s of recoverable headroom, re-homed to Story 12.4**, against the ~2.7 s the entire engine choice was worth. **Only a reference-device run can bind this; a PC POC number is never a mobile number.** Full evidence, traceability and the coverage caveat: [`architecture-spike-gpu-megashader.md`](architecture-spike-gpu-megashader.md).
...
- **PERF-010 (mobile):** Reference-device performance — **soft target, not a measured floor.** V1 targets the Poco X5 Pro 5G (SM7325 / Snapdragon 778G class) as the dev reference device but does **not** commit a numeric minimum-supported-device floor. The original plan to bind a measured number via the AR-SPIKE follow-up (Story 1.1.1) was cancelled on 2026-05-09 in favour of a build-and-observe approach (see `_bmad-output/architecture-spike-perf-floor.md` decision banner). **Amended 2026-07-16; DISPOSITION RECORDED 2026-09-15 (Story 12.3) — PERF-010 STAYS A SOFT TARGET. Story 12.2's reference-device run does NOT bind a number here.** [`architecture-spike-gpu-megashader.md`](architecture-spike-gpu-megashader.md) is published and reports measured reference-device numbers, and it deliberately declines to bind this NFR: what it measured is **detection-pass throughput on one device**, whereas a minimum-supported-device floor is a claim about the device **population**, which a single-device run cannot produce. Binding PERF-010 would require a device matrix, which no story owns and none is proposed. The 2026-05-09 ship-and-observe softening is a **product decision, not a measurement outcome**, and nothing Epic 12 measured reverses it. *(What the run does contribute is a documented **capability point**, recorded as evidence and not as a floor: SM7325 / Adreno 642L / Android 14 completes the detection pass at ~1% of source duration with no thermal throttling observed over an ~8-minute run. Note **PERF-002, by contrast, IS now re-baselined from measurement** — see above.)* V1 launch is **not gated** on a measurement; it ships against whatever device set the Play Store filter accepts and is revisited only if real-user feedback surfaces unacceptable performance. Cloud-fallback CV remains FORBIDDEN regardless (architecture AC11). *(Section 2, Section 5)*
