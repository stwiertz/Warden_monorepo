...
- [ ] **Task 1: Create `POST /api/subscription/portal` route** (AC: #1, #3, #4)
  - [ ] 1.1 Create `src/app/api/subscription/portal/route.ts`. This sits alongside the existing `GET /api/subscription` route ([src/app/api/subscription/route.ts](src/app/api/subscription/route.ts)).
  - [ ] 1.2 Use `withAuth` from [src/lib/firebase/auth.ts](src/lib/firebase/auth.ts) — same pattern as the existing subscription route and checkout session route. This handles session cookie validation and returns 401 on failure.
  - [ ] 1.3 Implementation:
    ```ts
    import 'server-only'
    ...
    ```
  - [ ] 1.4 The route reads `stripe_customer_id` from Firestore, NOT from a request body. The customer ID is an internal field — the client should never send it.
  - [ ] 1.5 Use `getStripe()` from [src/lib/stripe/server.ts](src/lib/stripe/server.ts) — the existing singleton Stripe instance (SDK v22.0.1, API version `2026-03-25.dahlia`).
  - [ ] 1.6 The `return_url` is set to `/dashboard` so the subscriber lands back on the dashboard after leaving the portal. Use the same `NEXT_PUBLIC_APP_URL` resolution pattern from [checkout session/route.ts](src/app/api/checkout/session/route.ts) (lines 36-39).
  - [ ] 1.7 **Do NOT** pass `configuration` to `billingPortal.sessions.create()` — use the default portal configuration set in the Stripe Dashboard. This keeps the portal config in one place (Stripe Dashboard), not split between code and dashboard.
  - [ ] 1.8 **Do NOT** retry the Stripe call with `retryStripeCall`. Portal session creation is a one-time, user-initiated action — if it fails, the user clicks again. Retry logic is for webhook processing (Stories 4.2/4.3) where there's no user to retry.
  - [ ] 1.9 Include `import 'server-only'` at the top to prevent accidental client-side bundling of `adminDb` and Stripe SDK.

- [ ] **Task 2: Add "Manage Subscription" button to SubscriptionCard** (AC: #1, #4)
  - [ ] 2.1 Edit [src/components/dashboard/SubscriptionCard.tsx](src/components/dashboard/SubscriptionCard.tsx).
  - [ ] 2.2 Add a "Manage Subscription" button inside the subscription-data state (when `subscription` is not null and not in error/loading state). Place it below the `<dl>` description list, inside a `<div className="mt-4">` wrapper.
  - [ ] 2.3 The button calls the portal API and redirects:
    ```tsx
    ...
    ```
  - [ ] 2.4 Button styling: use `variant="outline"` (secondary action per UX button hierarchy — "Manage billing" is a supporting action, not the page's primary CTA). Full-width on mobile. Show loading spinner text ("Loading...") when `portalLoading` is true.
  - [ ] 2.5 Display `portalError` as a small red error text below the button if the portal API call fails. Matches the error display pattern already used in the error state of SubscriptionCard.
  - [ ] 2.6 **Do NOT** show the "Manage Subscription" button in the no-subscription state or error state — only when the subscriber has an active/past_due/canceled subscription.
  - [ ] 2.7 Add `import { useState } from 'react'` at the top (the component already has `'use client'` directive).

- [ ] **Task 3: Update HeaderAuthActions for signed-in navigation** (AC: #2)
  - [ ] 3.1 Edit [src/components/layout/HeaderAuthActions.tsx](src/components/layout/HeaderAuthActions.tsx).
  - [ ] 3.2 When the user is signed in (`user` is truthy), render a "Dashboard" link pointing to `/dashboard` **in addition to** the existing SignOutButton. The Dashboard link replaces the need for "Home" and "Pricing" nav links for signed-in users — per UX spec line 715: header nav shows "Dashboard" link when signed in.
  - [ ] 3.3 Updated signed-in rendering:
    ```tsx
    if (user) {
      return (
        <>
  ...
  - [ ] 3.6 Updated Header.tsx nav section:
    ...
    ```
  - [ ] 3.7 Updated HeaderAuthActions full rendering:
  ...
  - [ ] 3.8 The link styles (`text-muted-foreground hover:text-foreground ...`) are identical to what's currently in Header.tsx — copy them exactly. Use the same `className` string from the current static links.
  - [ ] 3.9 **Do NOT** change the Warden logo/home link on the left side of the header — that always links to `/`.

- [ ] **Task 4: Write portal API route tests** (AC: #5)
  - [ ] 4.1 Create `src/app/api/subscription/portal/route.test.ts`.
  - [ ] 4.2 Mock `@/lib/firebase/auth` (provide `withAuth` that invokes the handler), `@/lib/firebase/admin` (provide `adminDb.collection().doc().get()` returning controlled snapshots), and `@/lib/stripe/server` (provide `getStripe()` returning a mock with `billingPortal.sessions.create`). Follow mock patterns from [src/app/api/subscription/route.test.ts](src/app/api/subscription/route.test.ts) and [src/app/api/checkout/session/route.test.ts](src/app/api/checkout/session/route.test.ts).
  - [ ] 4.3 Test cases:
  ...
  - [ ] 4.4 Verify the `return_url` passed to `billingPortal.sessions.create` ends with `/dashboard`.
  - [ ] 4.5 Verify the `customer` param matches the Firestore `stripe_customer_id` value.

- [ ] **Task 5: Update HeaderAuthActions tests** (AC: #5)
  - [ ] 5.1 Edit [src/components/layout/HeaderAuthActions.test.tsx](src/components/layout/HeaderAuthActions.test.tsx).
  - [ ] 5.2 Update existing tests to account for the new rendering:
  ...
  - [ ] 5.3 Add a test: signed-in state shows "Dashboard" link pointing to `/dashboard`.
  - [ ] 5.4 Add a test: signed-in state does NOT show "Home", "Pricing", or "Sign in" links.

- [ ] **Task 6: Update Header tests** (AC: #5)
  - [ ] 6.1 Edit [src/components/layout/Header.test.tsx](src/components/layout/Header.test.tsx). Read the file first to understand current test structure.
  - [ ] 6.2 The Header component no longer renders static "Home" and "Pricing" links (those moved to HeaderAuthActions). Update or remove tests that assert those links exist directly in Header. The Warden logo link to `/` should still be tested.
  - [ ] 6.3 Header.tsx still imports HeaderAuthActions — ensure the mock for that component still works.

- [ ] **Task 7: Write SubscriptionCard portal button tests** (AC: #5)
  - [ ] 7.1 Edit [src/components/dashboard/SubscriptionCard.test.tsx](src/components/dashboard/SubscriptionCard.test.tsx). Read the file first.
  - [ ] 7.2 Add test cases:
  ...
  - [ ] 7.3 Mock `fetch` globally for portal API tests (`vi.stubGlobal('fetch', vi.fn())`).

- [ ] **Task 8: Update dashboard page tests** (AC: #5)
  - [ ] 8.1 Read [src/app/dashboard/page.test.tsx](src/app/dashboard/page.test.tsx) to check if any tests need updating due to SubscriptionCard changes.
  - [ ] 8.2 The SubscriptionCard mock in page tests should still work — the page passes props to SubscriptionCard, and the card's internal portal button behavior is tested in SubscriptionCard.test.tsx. Page tests should not need changes unless the component interface (props) changed — and it hasn't (no new props).
  - [ ] 8.3 Verify all 6 existing page tests still pass with no modifications.

- [ ] **Task 9: Full-suite regression pass** (AC: #5)
  - [ ] 9.1 `npx vitest run` — full suite green. Baseline: 308 tests across 40 files. Target: ~325-335 (new portal route tests + updated header tests + subscription card portal tests).
  - [ ] 9.2 `npm run lint` — 0 errors, 0 warnings.

- [ ] **Task 10: `npm run build`** (AC: #5)
  - [ ] 10.1 `npm run build` — zero type errors.
  - [ ] 10.2 Confirm `/api/subscription/portal` appears as `ƒ (Dynamic)` in the route manifest (new route).
  - [ ] 10.3 Confirm existing routes (`/api/subscription`, `/api/webhooks/stripe`, `/api/auth/session`, `/api/checkout/session`) still appear in manifest.

- [ ] **Task 11: Manual smoke test** (AC: #1, #2, human-verified)
  - [ ] 11.1 Start the dev server (`npm run dev`).
  - [ ] 11.2 **Test header — signed out:** Visit landing page. Verify header shows: Warden logo, Home link, Pricing link, Sign in link.
  - [ ] 11.3 **Test header — signed in:** Sign in. Verify header shows: Warden logo, Dashboard link, Sign out button. Verify Home and Pricing links are NOT shown.
  - [ ] 11.4 **Test Dashboard link:** Click "Dashboard" in header -> navigates to `/dashboard`.
  - [ ] 11.5 **Test "Manage Subscription" button:** On dashboard with an active subscription, click "Manage Subscription". Verify:
    - Button shows loading state briefly
    - Browser redirects to Stripe Customer Portal (billing.stripe.com)
    - Portal shows subscription details, plan switching options, cancellation, payment method updates
  - [ ] 11.6 **Test portal return:** Click the return link in Stripe Portal. Verify landing back on `/dashboard`.
  - [ ] 11.7 **Test no-subscription state:** Dashboard without subscription should NOT show "Manage Subscription" button.
  - [ ] 11.8 **Keyboard accessibility:** Tab through header nav links. Tab to "Manage Subscription" button. All should be focusable with visible orange focus ring.
  - [ ] 11.9 Document results in Completion Notes.
