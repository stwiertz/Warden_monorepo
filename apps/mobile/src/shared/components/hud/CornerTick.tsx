 ...
 */
export function CornerTick({ pos, inset = 10, size = 14, bottomOffset = 0 }: CornerTickProps) {
  const base = { position: "absolute" as const, width: size, height: size };
  const color = HUD.whiteDim;
  switch (pos) {
    case "tl":
      return (
        <View style={[base, { top: inset, left: inset, borderTopWidth: 1, borderLeftWidth: 1, borderColor: color }]} />
      );
    case "tr":
      return (
        <View style={[base, { top: inset, right: inset, borderTopWidth: 1, borderRightWidth: 1, borderColor: color }]} />
      );
    case "bl":
      return (
        <View
