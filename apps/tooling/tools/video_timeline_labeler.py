...
def _prompt_hud_version(video_path: str | None = None) -> str | None:
    ...
    # First-frame preview (best-effort).
...
class VideoTimelineLabelerApp(tk.Tk):
    ...
    def __init__(
        ...
        self._duration_s = self._frame_count / self._fps if self._fps else 0.0
    ...
    def _render_current_frame(self) -> None:
        ...
        if not ret:
            # Re-seek to current cursor and try once more before giving up
    ...
    def _label_current(self, class_name: str) -> None:
        ...
        primary_path = self._write_label_png(class_name, snapped)
        ...
        backfilled = 0
        for pts in between:
            fill_path = self._write_label_png(class_name, pts, log_prefix="backfill")
    ...
    def _write_label_png(
        self, class_name: str, snapped_pts: float, log_prefix: str = ""
    ) -> str | None:
        ...
        try:
            ...
            return None
        if not cv2.imwrite(dest_path, frame):
            self.bell()
            self._status_label.config(text=f"⚠ imwrite failed: {dest_path}")
            return None
...
def main(argv: list[str] | None = None) -> int:
    ...
    keyframe_pts = get_keyframe_timestamps(
        video_path, scan_duration=int(duration) + 1
    )
    print(
