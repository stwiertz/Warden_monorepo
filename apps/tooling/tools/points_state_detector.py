"""Points State Detector — CLI tool for classifying in-game vs lobby frames.

Iterates through I-frames of an EVA session recording, checks the 'points' ROI
for white pixels (score text) using HSV thresholds. White text present = in-game,
absent = lobby/loading. Detects transitions and exports start/end-of-round frames
as PNGs, matching the same output format as the black screen detector.

Usage:
    python tools/points_state_detector.py <video_path> [-o OUTPUT_DIR] [-c CONFIG] [--profile]
"""
...
def run(video_path, output_dir, config, profile=False, roi_name="points"):
    """Run points-based state detection on a video file.
    ...
    # Find the target ROI from config
    ...
    if points_roi_raw is None:
        raise ValueError(
            f"Config must define a '{roi_name}' ROI zone. "
    ...
    print(f"Config: sat_max={sat_max}, val_min={val_min}, min_ratio={min_ratio}, "
          ...
          f"end_confirm={end_confirm_frames}, target_height={target_height}px")
    print(f"ROI: {roi_name} ({points_roi_raw['x']},{points_roi_raw['y']} "
        ...
        if state == "not_in_game":
            ...
            if not white_detected:
                ...
                if end_confirm_count >= end_confirm_frames:
                    ...
                    end_confirm_count = 0
                    end_candidate_timestamp = None
                    print(f"  END at {confirmed_end_ts:.1f}s")
            else:
                # White still present — reset end confirmation
                end_confirm_count = 0
                end_candidate_timestamp = None
    ...
    if state == "in_game":
        print("Warning: Video ended while in-game — last round has no end transition.", file=sys.stderr)
...
def main():
    parser = argparse.ArgumentParser(
        description="Detect in-game vs lobby transitions via points ROI white detection."
    )
    parser.add_argument(
        "video",
    ...
    parser.add_argument(
        "--roi",
        default="points",
        help="Name of the ROI zone to use for white detection (default: points)",
    )
    parser.add_argument(
        "--profile",
        action="store_true",
