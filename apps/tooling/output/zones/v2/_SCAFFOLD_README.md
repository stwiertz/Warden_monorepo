...
>
> 🔴 **Those three numbers are IN-SAMPLE — a parity reference, not a generalization claim.**
> The zones were tuned against the very corpus they are scored on (the low-saturation
> decision below moved map-ID 0.973 → 1.000 by setting `h_tol=180` on 65 of 121 zones), and
> there is **no holdout**. Two classifiers at exactly **1.000** is what a fit to its own
> training data looks like. They are the right target for **GPU-vs-CPU frame parity**
> (Story 12.1) and are **not** evidence of shipped detection quality — REL-006's ≥95% floor
> is not gated by them. Measuring generalization is **Story 9.9b's** job (its AC2
> certitude + zero-train/test-overlap gate). *(Label added 2026-07-16, Story 9.15 code review.)*
>
...
> ⚠️ **Census below is the 2026-05-17 snapshot and is STALE — see the corrections header at
> the top of this file for live figures.** Left verbatim as the dated record of Stephane's
> operative interpretation (editing a completed record destroys the audit trail). Deltas:
> negatives are **380** live (`296+41+43`), not 381 — `transition` is **43**, not 44; and the
> per-map counts moved (`the_rock` 98→**38**, `artefact` 432→**418**), pushing the small-sample
> cohort from 5 maps to **5 maps with the_rock materially smaller**. **The interpretation itself
> still holds** — the negative pool is ample either way and the certitude gate is unchanged.
...
- `manifest.json` — human-authored. Required keys: `hud_version` (enum, `"v2"`),
  ...
  **DERIVE `manifest.json` alongside `manifest.template.json` — do NOT rename the template.**
  (Corrected 2026-07-16, Story 9.15 AC7: the template is a **tracked** file; renaming it
  deletes it from git. Copy it, fill the sentinel, save as `manifest.json`.) The
  `score_screen_duration_ms` sentinel is intentionally a string so it CANNOT silently pass
  the AC3 dry-run. ⚠️ The template hardcodes `hud_version: "v2"` — when scaffolding a NEW
  HUD version, change it. The emitter derives the output filename from this field alone and
  never cross-checks it against `--zones-dir`, so a forgotten edit silently overwrites
  `map_config.v2.json` from v3 fragments.
...
#        Captures ARE available: monorepo-root videos/V2/ (4 real EVA captures, 12.7 GB,
#        6h13m). NOT apps/tooling/source/. (Corrected 2026-07-16, Story 9.15 AC7.)
#        NOTE: only 4 captures exist, so the ">=5 median" floor above cannot be met today.
#        The shipped score_screen_duration_ms = 15000 is ACCEPTED AS-IS and UNVERIFIED.
...
# AC8 — end-to-end smoke. NOT BLOCKED (corrected 2026-07-16, Story 9.15 AC7): 4 real EVA
#        captures live at monorepo-root videos/V2/ (gitignored, 12.7 GB — local only).
uv run python tools/video_test.py ../../videos/V2/<capture.mp4> \
    --config output/map_configs/map_config.v2.json
```
